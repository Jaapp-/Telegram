...
public class StarGiftPatterns {
    ...
    public static void drawPatternAnimated(Canvas canvas, int type, Drawable pattern, float w, float h, float alpha, float scale, float progress) {
        if (alpha <= 0.0f) return;
        for (int i = 0; i < patternLocations[type].length; i += 4) {
            final float x = patternLocations[type][i];
            final float y = patternLocations[type][i + 1];
            ...
            float oneMinusT = 1 - thisProgress;
            float bezX = oneMinusT * oneMinusT * x0 + 2 * oneMinusT * thisProgress * x1 + thisProgress * thisProgress * x2;
            float bezY = oneMinusT * oneMinusT * y0 + 2 * oneMinusT * thisProgress * y1 + thisProgress * thisProgress * y2;
